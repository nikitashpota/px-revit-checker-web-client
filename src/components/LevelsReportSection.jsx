...
export default function LevelsReportSection({ modelId }) {
  ...
  const loadData = async () => {
    try {
      const res = await modelsAPI.getLevelsReport(modelId)
      setData(res)
    } catch (err) {
  ...
  const exportCSV = () => {
    ...
    const BOM = '\uFEFF'
    const headers = ['ElementId', 'Имя уровня', 'Типы ошибок', 'Смещение (мм)', 'Закреплён', 'Рабочий набор']
    ...
    a.href = url
    a.download = `level-errors-${modelId}.csv`
    a.click()
  }
  ...
  if (!data?.has_data) {
    return <div className="text-center py-8 text-gray-500">Данные проверки уровней отсутствуют</div>
  }
    ...
    <div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg p-4 shadow-sm border">
          <div className="text-2xl font-bold text-gray-900">{data.total_levels_in_model}</div>
          <div className="text-sm text-gray-500">Всего уровней</div>
        </div>
        <div className="bg-white rounded-lg p-4 shadow-sm border">
        ...
        <div className="bg-white rounded-lg p-4 shadow-sm border">
          <div className="text-2xl font-bold text-blue-600">{data.total_reference_levels}</div>
      ...
      {errors.length > 0 ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-medium text-gray-500">Имя уровня</th>
                  ...
                  <th className="px-4 py-3 text-left font-medium text-gray-500">Закреплён</th>
              ...
              <tbody className="divide-y">
                {paginatedErrors.map((e, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">{e.level_name}</td>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
          ...
          <div className="text-green-700 font-medium">Все уровни соответствуют эталону</div>
        </div>
      )}
    </div>
  )
